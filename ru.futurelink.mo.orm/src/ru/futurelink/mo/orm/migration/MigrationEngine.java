...
public class MigrationEngine {
	...
	private PersistentManager	mPersistentManager;
	...
	public void start() {
		...
		mPersistentManager = (PersistentManager) mBundleContext.getService(ref);		
		
		TypedQuery<Integer> currentVersionQuery = mPersistentManager.getEm().createQuery(
		...
		mPersistentManager.logger().info("Текущая версия базы: {}", currentVersion);
		...
		if (migrationsFromVersion == null) {
			mPersistentManager.logger().info("Нет доступных миграций с этой версии базы данных. Завершено.");
			return;
		}
		...
		for (Class<? extends Migration> migrationClass : migrationsFromVersion.keySet()) {
			...
			// все.
			mPersistentManager.logger().info("Получили миграцию с реализацией в {}", migrationClass.getName());
			...
			try {
				Constructor<? extends Migration> ctr = (Constructor<? extends Migration>) migrationClass.getConstructor(
					PersistentManager.class, int.class, int.class);
				migrationInstance = ctr.newInstance(mPersistentManager, currentVersion, versionTo);
			...
			mPersistentManager.logger().info("Отработали миграцию с реализцией в {}", migrationClass.getName());
		}
		
		mPersistentManager.logger().info("Все миграции отработаны.");
	}
